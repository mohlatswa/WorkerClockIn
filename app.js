...
function renderAutoPayslip(p) {
  var win = window.open('', '_blank', 'width=480,height=720');
  ...
  win.document.write('<html><head><title>Payslip — ' + esc(p.period_label) + '</title><style>' +
    'body{font-family:Inter,Arial,sans-serif;margin:0;padding:26px;color:#0F172A}' +
    '.ps{max-width:420px;margin:0 auto;border:1px solid #E2E8F0;border-radius:14px;padding:22px}' +
    'h1{font-size:18px;color:#312E81;margin:0 0 2px}.sub{color:#64748B;font-size:13px;margin-bottom:16px}' +
    'table{width:100%;border-collapse:collapse;font-size:14px}td{padding:7px 0;border-bottom:1px solid #EEF2FF}' +
    '.r{text-align:right}.tot td{font-weight:800;font-size:16px;border-top:2px solid #312E81;border-bottom:none;padding-top:10px}' +
    '.foot{margin-top:16px;font-size:11px;color:#94A3B8}button{margin-top:16px;width:100%;padding:11px;border:none;border-radius:9px;background:#4F46E5;color:#fff;font-weight:700;font-size:14px;cursor:pointer}' +
    '</style></head><body><div class="ps">' +
    '<h1>' + esc(co) + '</h1><div class="sub">Payslip &middot; ' + esc(p.period_label) + '</div>' +
    '<table>' +
    '<tr><td>Employee</td><td class="r">' + esc(S.worker.name) + '</td></tr>' +
    '<tr><td>Regular hours</td><td class="r">' + reg.toFixed(1) + ' h</td></tr>' +
    '<tr><td>Overtime hours</td><td class="r">' + ot.toFixed(1) + ' h</td></tr>' +
    '<tr><td>Rate</td><td class="r">' + fmtR(rate) + '/h</td></tr>' +
    '<tr><td>Regular pay</td><td class="r">' + fmtR(regPay) + '</td></tr>' +
    '<tr><td>Overtime pay (' + mult + '×)</td><td class="r">' + fmtR(otPay) + '</td></tr>' +
    '<tr class="tot"><td>Gross pay</td><td class="r">' + fmtR(gross) + '</td></tr>' +
    '</table><div class="foot">Generated by WorkClock-In &middot; ' + esc(co) + '</div>' +

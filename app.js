    ...
    '<div class="sheet">' +
      ...
      '<div class="body">' +
        ...
        '<div class="note"><b>PAYE &amp; UIF are estimates</b> computed by WorkClock-In using the SARS ' + esc(taxYr) + ' tax tables ' +
          '(applicable age rebate; no medical-aid or other credits). Verify against the official SARS tables / your payroll before relying on them.<br>' +
  ...
  document.getElementById('payroll-rows').innerHTML = _payroll.list.map(function(w) {
    return '<tr data-key="' + escQ(w.key) + '">' +
      ...
      '<td><input class="input pay-rate" type="number" min="0" step="0.01" inputmode="decimal" ' +
        'value="' + (rates[w.key] != null ? rates[w.key] : '') + '" oninput="payrollRecalc()">' +
        '<select class="input pay-age" title="Tax age band (for PAYE rebate)" onchange="payrollRecalc()">' +
...
function payrollPDF() {
  ...
  document.getElementById('print-area').innerHTML =
    ...
    '<div class="pr-foot">PAYE &amp; UIF are estimates (SARS ' + TAX_YEAR + ', applicable age rebate) — verify before filing. Generated ' + new Date().toLocaleString('en-ZA', { timeZone: _tz() }) + ' · WorkClock-In by Reatlegile Solutions</div>';
  window.print();
}
function payrollCSV() {

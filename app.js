...
// ── State ─────────────────────────────────────────────
const S = {
  worker: null, admin: null, workplace: null,
  userLoc: null, geoWatcher: null,
  clockStatus: 'out', attendanceId: null,
  authMethod: 'pin', authSource: 'manual',
  scanStream: null, scanRunning: false,
  nfcReader: null, npPin: [], npTimer: null,
...
function toast(msg, dur=3000) {
  ...
  clearTimeout(_toastT); _toastT = setTimeout(() => el.classList.add('hidden'), dur);
}
...
function updateHomeUI() {
  ...
  if (S.companyId) {
    ...
    noCard?.classList.add('hidden'); methods?.classList.remove('hidden');
  } else {
    if (nameEl)  nameEl.textContent = '';
    noCard?.classList.remove('hidden'); methods?.classList.add('hidden');
  }
}
  ...
  if (code) {
    try {
      const { data } = await withTimeout(
        db.from('companies').select('*').eq('code', code.toUpperCase()).eq('is_active', true).maybeSingle(), 4000
      );
...
async function openQR() {
  ...
  try {
    ...
    S.scanRunning = true; requestAnimationFrame(qrFrame);
  } catch (err) {
...
function stopQR() {
  S.scanRunning = false;
  S.scanStream?.getTracks().forEach(t => t.stop()); S.scanStream = null;
}
  ...
  try {
    ...
    S.nfcReader.addEventListener('reading', ({message}) => {
      ...
      for (const rec of message.records) {
        if (rec.recordType === 'text') { vibrate(80); stopNFC(); handleCardRead(dec.decode(rec.data).trim().toUpperCase()); return; }
      }
...
async function findWorker(overrideId) {
  ...
  try {
    ...
    if (error?.code === 'PGRST116') { showErr('err-empid','Multiple accounts — open your employer's link.'); return; }
    if (!data) {
      if (S.authSource === 'card') { toast('❌ Card not recognised (' + id + ')'); showPg('home'); }
      else showErr('err-empid','Employee ID not found. Check with your manager.');
      return;
    }
    S.worker = data; goToAuth(data);
  } catch {
    if (S.authSource === 'card') { toast('❌ Connection error'); showPg('home'); }
...
function goToAuth(w) {
  ...
  if (w.biometric_enabled && w.biometric_credential_id && window.PublicKeyCredential) {
    ...
    if (S.authSource === 'card') { showPg('auth'); npReset(); setTimeout(authBiometric,400); return; }
  } else { bioWrap.classList.add('hidden'); }
  showPg('auth'); npReset();
}

function backFromAuth() { S.worker=null; S.authSource='manual'; showPg('home'); }
  ...
  S.geoWatcher = navigator.geolocation.watchPosition(
    pos => {
      ...
      if (!wp?.latitude || !wp?.longitude) {
        document.getElementById('loc-status').innerHTML='⚠️ <span style="color:var(--amber)">Workplace not set — ask your admin to configure it</span>';
        setClockBtn(true); return;
      }
    ...
    err => {
      if (err.code===err.PERMISSION_DENIED) { locCard.style.display='none'; locBlk.classList.remove('hidden'); }
      else { document.getElementById('loc-status').textContent='⚠️ Location error: '+err.message; setClockBtn(false); }
    },
  ...
  try {
    if (action==='in') {
      const {data, error} = await db.from('attendance').insert({
        ...
        status: 'active',
        clock_in_latitude:  S.userLoc?.latitude  || null,
        clock_in_longitude: S.userLoc?.longitude || null,
      }).select().single();
      ...
      const {error} = await db.from('attendance').update({
        ...
        status: 'completed',
        clock_out_latitude:  S.userLoc?.latitude  || null,
        clock_out_longitude: S.userLoc?.longitude || null,
...
async function workerRegBio() {
  ...
  try {
    ...
    if (!cred) return;
    const {error} = await db.from('workers').update({biometric_credential_id:b64(cred.rawId),biometric_enabled:true}).eq('id',S.worker.id);
    if (error) throw error;
  ...
  } catch (err) { if (err.name!=='NotAllowedError') showMsg('bio-reg-msg','Error: '+err.message,'err'); }
}
...
async function adminLogin() {
  ...
  if (typeof db==='undefined') { showErr('err-admin','App not ready — please refresh the page.'); return; }
  ...
  try {
    ...
    if (data.role==='developer') {
      ...
      document.getElementById('admin-co-lbl').textContent = data.co?.name||'';
...
function toggleForgot() {
  ...
  if (!p.classList.contains('hidden')) {
    ['fp-s2','fp-s3'].forEach(id=>document.getElementById(id).classList.add('hidden'));
    document.getElementById('fp-s1').classList.remove('hidden');
...
async function sendOTP() {
  ...
  _otp.code=String(Math.floor(100000+Math.random()*900000)); _otp.expiry=Date.now()+600000;
  try {
  ...
  } catch (err) { showMsg('fp-msg','Failed: '+(err?.text||err.message),'err'); }
...
async function applyNewPw() {
  ...
  document.getElementById('fp-s3').classList.add('hidden');
...
async function loadDashboard() {
  ...
  const {data:recs}=ids.length
    ? await db.from('attendance').select('*, w:workers(name,employee_id)')
        .in('worker_id',ids).gte('clock_in_time',today.toISOString()).lt('clock_in_time',tmrw.toISOString())
  ...
  el.innerHTML=recs?.length
    ? recs.slice(0,15).map(r=>`<div class="act-item">
        <div><div class="act-name">${r.w?.name??'Unknown'}</div>
...
async function loadWorkers() {
  ...
  el.innerHTML='<div class="card" style="padding:0 18px">'+data.map(w=>`
    <div class="list-row">
      <div class="row-info">
        <div class="av av-sm">${initials(w.name)}</div>
        <div>
      ...
      <div class="row-btns">
        ...
        <button class="icon-btn" onclick="openCard('${w.id}','${w.employee_id}','${escQ(w.name)}','${escQ(w.job_title||'')}')">🪪</button>
...
async function addWorker() {
  ...
  if (error) { showMsg('nw-msg',error.message.includes('unique')?'Employee ID already exists.':error.message,'err'); return; }
...
async function adminRegBio(workerId,workerName) {
  ...
  try {
    ...
    if (!cred) return;
    const {error}=await db.from('workers').update({biometric_credential_id:b64(cred.rawId),biometric_enabled:true}).eq('id',workerId);
    if (error) throw error;
...
async function openCard(workerId,empId,name,job) {
  ...
  try { await QRCode.toCanvas(canvas,empId,{width:160,margin:2,color:{dark:'#1E293B',light:'#FFFFFF'}}); }
...
async function loadWorkerOptions() {
  ...
  sel.innerHTML='<option value="">All Workers</option>'+(data||[]).map(w=>`<option value="${w.id}">${w.name}${w.job_title?' · '+w.job_title:''} (${w.employee_id})</option>`).join('');
}

async function loadAttendance() {
  const from=document.getElementById('att-from').value;
  const to  =document.getElementById('att-to').value;
  const wkr =document.getElementById('att-worker').value;
  const mth =document.getElementById('att-method').value;
  ...
  try {
    ...
    const allowed=wkr?[wkr]:(cWks||[]).map(w=>w.id);
    ...
    showMsg('att-summary',data.length+' records · '+totalHrs.toFixed(1)+'h total · '+still+' still in','ok');
    el.innerHTML='<div class="card" style="padding:0 18px">'+data.map(r=>{
...
async function loadCoAdmins() {
  ...
  el.innerHTML='<div class="empty">Loading…</div>';
  const {data,error}=await db.from('admin_users').select('*').eq('company_id',S.admin?.company_id).neq('role','developer').order('full_name');
  if (error||!data) { el.innerHTML='<div class="empty">Failed to load</div>'; return; }
  if (!data.length) { el.innerHTML='<div class="empty">No admins yet — create one above</div>'; return; }
  el.innerHTML='<div class="card" style="padding:0 18px">'+data.map(a=>`
    <div class="list-row">
      <div class="row-info">
        <div class="av av-sm" style="background:${ROLE_COLORS[a.role]||'var(--blue)'}">${initials(a.full_name||a.username)}</div>
        <div>
      ...
      <div class="row-btns">
        ...
        <button class="icon-btn" onclick="resetPw('${a.id}','${a.username}')">🔑</button>
...
async function addAdmin() {
  ...
  const {error}=await db.from('admin_users').insert({username:user,password_hash:pass,full_name:name,email:email||null,role,company_id:S.admin?.company_id,is_active:true});
  if (error) { showMsg('ca-msg',error.message.includes('unique')?'Username already exists.':error.message,'err'); return; }
...
function copyClockInLink() {
  ...
  navigator.clipboard.writeText(link)
    .then(()=>showMsg('link-msg','✅ Link copied!','ok'))
...
async function saveEditWorker() {
  ...
  if (error) { showMsg('ewk-msg',error.message.includes('unique')?'Employee ID already in use.':error.message,'err'); return; }
  showMsg('ewk-msg','✅ Worker updated!','ok');
  setTimeout(()=>{ closeModal('modal-edit-worker'); if(ctx==='dev')loadDevWorkers();else loadWorkers(); },1200);
}
...
function openEditAcct(id,name,email,role,ctx) {
  ...
  const wrap=document.getElementById('eac-role-wrap');
...
async function saveEditAcct() {
  ...
  setTimeout(()=>{ closeModal('modal-edit-acct'); if(ctx==='dev')loadDevAccounts();else loadCoAdmins(); },1200);
}
...
async function saveEditCo() {
  ...
  if (error) { showMsg('eco-msg',error.message.includes('unique')?'Code already exists.':error.message,'err'); return; }
  showMsg('eco-msg','✅ Company updated!','ok');
  setTimeout(()=>{ closeModal('modal-edit-co'); loadDevCos(); },1200);
}
...
function switchDevTab(btn,name) {
  ...
  btn.classList.add('active');
  document.querySelectorAll('#pg-developer .tab-pane').forEach(p=>p.classList.remove('active'));
  document.getElementById(name).classList.add('active');
...
async function loadDevCos() {
  ...
  el.innerHTML='<div class="card" style="padding:0 18px">'+data.map(c=>`
    <div class="list-row">
      <div class="row-info">
        <div class="av av-sm" style="background:var(--purple)">${c.code.slice(0,2)}</div>
        <div><div class="row-name">${c.name}</div><div class="row-meta">Code: ${c.code}${!c.is_active?' · <em>Inactive</em>':''}</div></div>
      </div>
      <div class="row-btns">
...
async function addCompany() {
  ...
  if (error) { showMsg('nc-msg',error.message.includes('unique')?'Code already exists.':error.message,'err'); return; }
...
async function loadDevAccounts() {
  ...
  if (filterSel.options.length<=1) {
    ...
    filterSel.innerHTML='<option value="">All Companies</option>'+(cos||[]).map(c=>`<option value="${c.id}">${c.name}</option>`).join('');
  }
...
function toggleAddDevAcct() {
  ...
  if (!p.classList.contains('hidden')) {
    db.from('companies').select('id,name').eq('is_active',true).order('name').then(({data})=>{
      document.getElementById('da-company').innerHTML='<option value="">Select Company…</option>'+(data||[]).map(c=>`<option value="${c.id}">${c.name}</option>`).join('');
    });
...
async function addDevAcct() {
  ...
  const {error}=await db.from('admin_users').insert({username:user,password_hash:pass,full_name:name,email:email||null,role,company_id:cid,is_active:true});
  if (error) { showMsg('da-msg',error.message.includes('unique')?'Username already exists.':error.message,'err'); return; }
  ...
  setTimeout(()=>{ toggleAddDevAcct(); loadDevAccounts(); },1400);
}
...
async function loadDevWorkers() {
  ...
  if (sel.options.length<=1) {
    const {data:cos}=await db.from('companies').select('id,name').eq('is_active',true).order('name');
    sel.innerHTML='<option value="">Select a company…</option>'+(cos||[]).map(c=>`<option value="${c.id}">${c.name}</option>`).join('');
  }
  const cid=sel.value;
...
function loadDevSystem() {
  const el=document.getElementById('dev-acct-info'); if (!S.admin) return;
  el.innerHTML=`
...
let _faceStream=null, _faceRunning=false, _faceMatcher=null, _faceWorkerMap={};
...
async function openFaceRecog() {
  ...
  try { await loadFaceModels(); } catch { statusEl.textContent='❌ Could not load models — check internet connection.'; return; }
  statusEl.textContent='Loading enrolled faces…';
  const {data:workers}=await db.from('workers').select('id,name,employee_id,face_descriptor')
  ...
  for (const w of workers) {
    try { labeled.push(new faceapi.LabeledFaceDescriptors(w.id,[new Float32Array(JSON.parse(w.face_descriptor))])); _faceWorkerMap[w.id]=w; } catch {}
  }
  ...
  } catch (err) { statusEl.textContent=err.name==='NotAllowedError'?'❌ Camera access denied':'❌ '+err.message; }
}
...
async function faceFrame() {
  ...
  try {
    ...
    const {data}=await db.from('workers').select('*, workplace:workplaces(*)').eq('id',match.label).eq('is_active',true).maybeSingle();
    if (!data) { statusEl.textContent='❌ Account not found.'; setTimeout(()=>showPg('home'),2500); return; }
...
// ── Face Enrolment ────────────────────────────────────
let _enrollStream=null, _enrollTarget=null;
...
async function adminEnrollFace(workerId,workerName,ctx) {
  ...
  try { await loadFaceModels(); } catch { document.getElementById('enroll-status').textContent='❌ Could not load models.'; return; }
  ...
  } catch (err) { document.getElementById('enroll-status').textContent=err.name==='NotAllowedError'?'❌ Camera access denied':'❌ '+err.message; }
}
...
async function captureEnroll() {
  ...
  try {
    ...
    setTimeout(()=>{ closeFaceEnroll(); if(_enrollTarget.ctx==='dev')loadDevWorkers();else if(_enrollTarget.ctx==='admin')loadWorkers(); },1400);
...
window.addEventListener('beforeinstallprompt',e=>{ e.preventDefault(); _installPrompt=e; setTimeout(()=>document.getElementById('install-banner')?.classList.remove('hidden'),3000); });
window.addEventListener('appinstalled',()=>{ document.getElementById('install-banner')?.classList.add('hidden'); _installPrompt=null; toast('✅ WorkClock installed!'); });
...
function checkIOSInstall() {
  if (/iphone|ipad|ipod/i.test(navigator.userAgent)&&!window.navigator.standalone) {
    setTimeout(()=>document.getElementById('ios-banner')?.classList.remove('hidden'),3000);
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let done=false;
  const finish=()=>{ if(done)return; done=true; showPg('home'); };
  ...
  (async()=>{
    ...
    try {
      const {data}=await withTimeout(
        db.from('workers').select('*, workplace:workplaces(*)').eq('id',savedId).eq('is_active',true).maybeSingle(), 4000
      );
      if (data) {
        S.worker=data;
        if (!S.companyId) {
          try {
            const {data:co}=await withTimeout(db.from('companies').select('*').eq('id',data.company_id).maybeSingle(),3000);
            if (co) setCompany(co,false);
          } catch {}
        }
        done=true; enterClockScreen(); return;
      }
    } catch {}

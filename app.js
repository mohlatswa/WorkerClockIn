...
async function enterWorkerDashboard() {
  var w = S.worker;
  // Shared device: a different worker signing in must not inherit the previous
  // worker's cached payslips/PDFs. Same-worker (offline restore) keeps its cache.
  var prevId = localStorage.getItem('wc_worker_id');
  if (prevId && prevId !== w.id) _psClearCache();
...
function renderPayslipRow(p) {
  var amt = p.nett != null ? fmtR(+p.nett) + ' nett' : (p.gross != null ? fmtR(+p.gross) : '');
  return '<div class="lv-row">' +
    '<div style="flex:1;min-width:0">' +
      '<div style="font-weight:600;font-size:.86rem">' + esc(p.period_label) + '</div>' +
      '<div class="sub" style="font-size:.77rem">' + (amt ? amt + ' · ' : '') +
        (p.source === 'upload' ? 'PDF' : 'Auto') + ' · ' + fmtDate(p.created_at) + '</div>' +
    '</div>' +
    '<button class="btn btn-outline btn-sm" onclick="openMyPayslip(\'' + p.id + '\')">' + icon('download', 15) + ' View</button>' +
  '</div>';
}
...
function renderAutoPayslip(p) {
  var win = window.open('', '_blank', 'width=820,height=960');
  if (!win) { toast('Allow pop-ups to view the payslip'); return; }
  var co  = S.companyName || 'WorkClock-In';
  var w   = S.worker || {};
  var reg = +p.reg_hours || 0, ot = +p.ot_hours || 0, rate = +p.rate || 0, mult = +p.ot_mult || 1.5;
  var regPay = reg * rate, otPay = ot * rate * mult, gross = p.gross != null ? +p.gross : (regPay + otPay);
  var totalHrs = reg + ot;
  // Deductions: use the figures stored at publish time; fall back to a live calc for older payslips.
  var paye, uif, nett;
  if (p.paye != null || p.nett != null) {
    paye = +p.paye || 0; uif = +p.uif || 0; nett = p.nett != null ? +p.nett : Math.max(0, gross - paye - uif);
  } else {
    var ded = saDeductions(gross, payPeriodsPerYear(p.period_start, p.period_end), 'u65');
    paye = ded.paye; uif = ded.uif; nett = ded.nett;
  }
  var taxYr = p.tax_year || TAX_YEAR;
  var period = p.period_start && p.period_end ? (fmtDate(p.period_start) + ' – ' + fmtDate(p.period_end)) : esc(p.period_label);
  var payDate = p.created_at ? fmtDate(p.created_at) : '';
  var mono = (co || 'W').trim().charAt(0).toUpperCase();
  function row(desc, hrs, rt, amt) {
    return '<tr><td>' + desc + '</td><td class="r">' + (hrs != null ? hrs.toFixed(2) : '') + '</td>' +
      '<td class="r">' + (rt != null ? fmtR(rt) : '') + '</td><td class="r">' + fmtR(amt) + '</td></tr>';
  }
  win.document.write('<!DOCTYPE html><html><head><meta charset="utf-8"><title>Payslip — ' + esc(w.name || '') + ' — ' + esc(p.period_label) + '</title>' +
    '<meta name="viewport" content="width=device-width,initial-scale=1"><style>' +
    '*{box-sizing:border-box}html,body{margin:0;background:#F1F5F9;color:#0F172A;font-family:Inter,"Segoe UI",Arial,sans-serif;-webkit-print-color-adjust:exact;print-color-adjust:exact}' +
    '.sheet{max-width:780px;margin:24px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 10px 40px rgba(15,23,42,.12)}' +
    '.hd{background:linear-gradient(135deg,#312E81 0%,#4F46E5 100%);color:#fff;padding:26px 34px;display:flex;justify-content:space-between;align-items:flex-start;gap:18px}' +
    '.brand{display:flex;align-items:center;gap:13px}.mono{width:46px;height:46px;border-radius:12px;background:rgba(255,255,255,.18);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:22px}' +
    '.brand h1{font-size:20px;margin:0;font-weight:800;letter-spacing:-.01em}.brand .t{font-size:12px;opacity:.85;margin-top:2px;letter-spacing:.08em;text-transform:uppercase}' +
    '.hd .pd{text-align:right;font-size:12.5px;line-height:1.5}.hd .pd b{display:block;font-size:11px;opacity:.8;font-weight:600;text-transform:uppercase;letter-spacing:.06em}' +
    '.body{padding:26px 34px 30px}' +
    '.meta{display:grid;grid-template-columns:1fr 1fr;gap:10px 28px;padding:18px 20px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:12px;margin-bottom:24px}' +
    '.meta .k{font-size:10.5px;text-transform:uppercase;letter-spacing:.06em;color:#64748B;font-weight:600}.meta .v{font-size:14px;font-weight:600;margin-top:1px}' +
    'h2{font-size:12px;text-transform:uppercase;letter-spacing:.07em;color:#4F46E5;margin:0 0 8px;font-weight:700}' +
    'table{width:100%;border-collapse:collapse;font-size:13.5px}thead th{text-align:left;font-size:10.5px;text-transform:uppercase;letter-spacing:.05em;color:#64748B;padding:8px 10px;border-bottom:2px solid #E2E8F0}' +
    'thead th.r,td.r{text-align:right}tbody td{padding:10px;border-bottom:1px solid #EEF2FF}' +
    'tfoot td{padding:10px;font-weight:700}.sub-tot td{border-top:1px solid #E2E8F0;color:#334155}' +
    '.summary{margin-top:22px;display:flex;justify-content:flex-end}' +
    '.netbox{min-width:280px;background:#EEF2FF;border:1px solid #C7D2FE;border-radius:12px;padding:16px 20px}' +
    '.netbox .line{display:flex;justify-content:space-between;font-size:13.5px;padding:3px 0;color:#334155}' +
    '.netbox .grand{display:flex;justify-content:space-between;align-items:baseline;border-top:2px solid #312E81;margin-top:8px;padding-top:10px}' +
    '.netbox .grand .lbl{font-size:13px;font-weight:700;color:#312E81}.netbox .grand .amt{font-size:24px;font-weight:800;color:#312E81}' +
    '.note{margin-top:20px;font-size:11px;color:#94A3B8;line-height:1.6;border-top:1px solid #E2E8F0;padding-top:14px}' +
    '.actions{max-width:780px;margin:0 auto 30px;display:flex;gap:10px;justify-content:flex-end;padding:0 4px}' +
    '.actions button{padding:11px 20px;border:none;border-radius:10px;background:#4F46E5;color:#fff;font-weight:700;font-size:13.5px;cursor:pointer;font-family:inherit}' +
    '.actions .ghost{background:#fff;color:#475569;border:1px solid #CBD5E1}' +
    '@media print{body{background:#fff}.sheet{box-shadow:none;margin:0;max-width:none;border-radius:0}.actions{display:none}@page{size:A4;margin:14mm}}' +
    '</style></head><body>' +
    '<div class="sheet">' +
      '<div class="hd">' +
        '<div class="brand"><div class="mono">' + esc(mono) + '</div><div><h1>' + esc(co) + '</h1><div class="t">Salary Advice</div></div></div>' +
        '<div class="pd"><b>Pay Period</b>' + period + (payDate ? '<div style="margin-top:6px"><b>Issued</b>' + payDate + '</div>' : '') + '</div>' +
      '</div>' +
      '<div class="body">' +
        '<div class="meta">' +
          '<div><div class="k">Employee</div><div class="v">' + esc(w.name || '') + '</div></div>' +
          '<div><div class="k">Employee No.</div><div class="v">' + esc(w.employee_id || '—') + '</div></div>' +
          '<div><div class="k">' + (w.job_title ? 'Position' : 'Period') + '</div><div class="v">' + esc(w.job_title || p.period_label) + '</div></div>' +
          '<div><div class="k">Hourly Rate</div><div class="v">' + fmtR(rate) + ' / h</div></div>' +
        '</div>' +
        '<h2>Earnings</h2>' +
        '<table><thead><tr><th>Description</th><th class="r">Hours</th><th class="r">Rate</th><th class="r">Amount</th></tr></thead>' +
        '<tbody>' +
          row('Regular pay', reg, rate, regPay) +
          (ot > 0 ? row('Overtime pay (' + mult + '×)', ot, rate * mult, otPay) : '') +
        '</tbody>' +
        '<tfoot><tr class="sub-tot"><td>Gross earnings</td><td class="r">' + totalHrs.toFixed(2) + '</td><td></td><td class="r">' + fmtR(gross) + '</td></tr></tfoot>' +
        '</table>' +
        '<h2 style="margin-top:22px">Deductions</h2>' +
        '<table><thead><tr><th>Description</th><th class="r">Amount</th></tr></thead>' +
        '<tbody>' +
          '<tr><td>PAYE (income tax)</td><td class="r">' + fmtR(paye) + '</td></tr>' +
          '<tr><td>UIF (1%)</td><td class="r">' + fmtR(uif) + '</td></tr>' +
        '</tbody>' +
        '<tfoot><tr class="sub-tot"><td>Total deductions</td><td class="r">' + fmtR(paye + uif) + '</td></tr></tfoot>' +
        '</table>' +
        '<div class="summary"><div class="netbox">' +
          '<div class="line"><span>Gross earnings</span><span>' + fmtR(gross) + '</span></div>' +
          '<div class="line"><span>Less deductions</span><span>− ' + fmtR(paye + uif) + '</span></div>' +
          '<div class="grand"><span class="lbl">NETT PAY</span><span class="amt">' + fmtR(nett) + '</span></div>' +
        '</div></div>' +
        '<div class="note"><b>PAYE &amp; UIF are estimates</b> computed by WorkClock-In using the SARS ' + esc(taxYr) + ' tax tables ' +
          '(primary rebate, under 65; no medical-aid or other credits). Verify against the official SARS tables / your payroll before relying on them.<br>' +
          'Generated by WorkClock-In for ' + esc(co) + '. This is a computer-generated document.</div>' +
      '</div>' +
    '</div>' +
    '<div class="actions"><button class="ghost" onclick="window.close()">Close</button><button onclick="window.print()">Save / Print PDF</button></div>' +
    '</body></html>');
  win.document.close();
}

// ── Admin Payslips (publish auto + upload PDF) ────────────
function payslipPeriodLabel(from, to) {
  try {
    var f = new Date(from), t = new Date(to);
    var mf = f.toLocaleDateString('en-ZA', { month: 'long', year: 'numeric' });
    var mt = t.toLocaleDateString('en-ZA', { month: 'long', year: 'numeric' });
    return mf === mt ? mf
      : f.toLocaleDateString('en-ZA', { day: 'numeric', month: 'short' }) + ' – ' + t.toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', year: 'numeric' });
  } catch (e) { return from + ' to ' + to; }
}
async function publishPayslips() {
  var rows = payrollRecalc(); if (!rows) return;
  var withRate = rows.filter(function(r) { return r.rate > 0 && r.w.key && /^[0-9a-f-]{36}$/i.test(r.w.key); });
  if (!withRate.length) { showMsg('payroll-msg', 'Enter at least one hourly rate first.', 'err'); return; }
  if (!confirm('Publish ' + withRate.length + ' payslip(s) for ' + _payroll.from + ' → ' + _payroll.to + '? Each worker will be able to view & download their own.')) return;
  var label = payslipPeriodLabel(_payroll.from, _payroll.to);
  var ok = 0, fail = 0;
  ...
  showMsg('payroll-msg', 'Published ' + ok + ' payslip(s)' + (fail ? ' · ' + fail + ' failed' : '') + '. Staff can now see them.', fail ? 'err' : 'ok');
}
async function openUploadPayslip() {
  var cid = requireAdminCid(); if (!cid) return;
  ...
  document.getElementById('ups-period').value = '';
  document.getElementById('ups-start').value = '';
  document.getElementById('ups-end').value = '';
  document.getElementById('ups-file').value = '';
  document.getElementById('ups-msg').classList.add('hidden');
  document.getElementById('modal-upload-payslip').classList.remove('hidden');
}
async function uploadPayslip() {
  var wid = document.getElementById('ups-worker').value;
  var period = document.getElementById('ups-period').value.trim();
  var start = document.getElementById('ups-start').value || null;
  var end = document.getElementById('ups-end').value || null;
  var fileEl = document.getElementById('ups-file');
  var file = fileEl.files && fileEl.files[0];
  if (!wid) { showMsg('ups-msg', 'Pick a worker.', 'err'); return; }
  if (!period) { showMsg('ups-msg', 'Enter a period label (e.g. June 2026).', 'err'); return; }
  if (!file) { showMsg('ups-msg', 'Choose a PDF file.', 'err'); return; }
  if (file.size > 4 * 1024 * 1024) { showMsg('ups-msg', 'File too large (max 4 MB).', 'err'); return; }
  var btn = document.getElementById('ups-btn'); btn.disabled = true;
  try {
    var b64 = await fileToBase64(file);
    var res = await withTimeout(db.rpc('admin_upload_payslip', {
...
function renderAdminPayslipRow(p) {
  ...
  return '<div class="aps-row">' +
    ...
    '</div>' +
    '<div class="aps-amt">' + (p.gross != null ? fmtR(+p.gross) : '—') +
      (p.nett != null ? '<div class="aps-nett">' + fmtR(+p.nett) + ' nett</div>' : '') + '</div>' +
...
// ── South African PAYE / UIF engine ────────────────────────
// ⚠️ STATUTORY TABLES — update here each tax year and VERIFY against the
// official SARS tables (and cross-check one payslip against PaySpace).
// Figures below are the SARS 2025/26 tax year. PAYE assumes the primary
// rebate (under 65) and standard pay frequencies; no medical-aid credits.
var TAX_YEAR = '2025/26';
var SARS_BRACKETS = [ // annual taxable income: cap, base tax at start of band, marginal rate
...
function saAnnualTax(annual) {
  for (var i = 0; i < SARS_BRACKETS.length; i++) {
    var b = SARS_BRACKETS[i];
    if (annual <= b.upTo) { var prev = i ? SARS_BRACKETS[i - 1].upTo : 0; return b.base + (annual - prev) * b.rate; }
  }
  return 0;
}
function saRebate(ageBand) {
  var r = SARS_REBATE.primary;
  if (ageBand === '65' || ageBand === '75') r += SARS_REBATE.secondary;
  if (ageBand === '75') r += SARS_REBATE.tertiary;
  return r;
}
// Standard SARS frequencies from the period length (snaps weekly/fortnightly/monthly).
function payPeriodsPerYear(fromStr, toStr) {
  try {
    var d = (new Date(toStr) - new Date(fromStr)) / 86400000 + 1; // inclusive days
    if (!(d > 0)) return 12;
    if (d <= 9)  return 52;   // weekly
    if (d <= 17) return 26;   // fortnightly
...
function saUif(periodGross, ppy) {
  ...
  var monthly = ppy > 0 ? periodGross * ppy / 12 : periodGross;
...
function payrollRecalc() {
  ...
  var rates = {}, total = 0, totalNett = 0, computed = [];
  ...
  rows.forEach(function(tr, i) {
    ...
    var ded = saDeductions(gross, ppy, 'u65');
    ...
    computed.push({ w: w, rate: rate, regPay: regPay, otPay: otPay, gross: gross, paye: ded.paye, uif: ded.uif, nett: ded.nett });
  });
...
function payrollPDF() {
  ...
  var body = rows.map(function(r) {
    var nett = r.nett != null ? r.nett : r.gross;
    return '<tr><td>' + esc(r.w.name) + '</td><td>' + esc(r.w.emp || '') + '</td>' +
      '<td class="r">' + r.w.reg.toFixed(1) + '</td><td class="r">' + r.w.ot.toFixed(1) + '</td>' +
      '<td class="r">' + fmtR(r.rate) + '</td><td class="r">' + fmtR(r.gross) + '</td>' +
      '<td class="r">' + fmtR(r.paye || 0) + '</td><td class="r">' + fmtR(r.uif || 0) + '</td>' +
      '<td class="r">' + fmtR(nett) + '</td></tr>';
  }).join('');
  var tReg = 0, tOt = 0, tPaye = 0, tUif = 0;
  rows.forEach(function(r) { tReg += r.w.reg; tOt += r.w.ot; tPaye += (r.paye || 0); tUif += (r.uif || 0); });
  var sh = curShift();
  document.getElementById('print-area').innerHTML =
    '<div class="pr-head"><h1>' + esc((S.admin && S.admin.co_name) || 'WorkClock-In') + '</h1>' +
    '<div class="pr-title">Payroll Summary</div>' +
    '<div class="pr-sub">' + esc(_payroll.from) + ' to ' + esc(_payroll.to) + ' &middot; OT paid at ' + _payroll.mult + '× &middot; PAYE/UIF per SARS ' + TAX_YEAR + '</div></div>' +
    '<table class="pr-table"><thead><tr><th>Worker</th><th>Emp ID</th><th class="r">Reg h</th><th class="r">OT h</th><th class="r">Rate</th><th class="r">Gross</th><th class="r">PAYE</th><th class="r">UIF</th><th class="r">Nett</th></tr></thead>' +
    '<tbody>' + body + '</tbody>' +
    '<tfoot><tr><td colspan="2">Total — ' + rows.length + ' workers</td><td class="r">' + tReg.toFixed(1) + '</td><td class="r">' + tOt.toFixed(1) + '</td><td></td><td class="r">' + fmtR(_payroll.total) + '</td><td class="r">' + fmtR(tPaye) + '</td><td class="r">' + fmtR(tUif) + '</td><td class="r">' + fmtR(_payroll.totalNett) + '</td></tr></tfoot></table>' +
    '<div class="pr-foot">PAYE &amp; UIF are estimates (SARS ' + TAX_YEAR + ', primary rebate / under 65) — verify before filing. Generated ' + new Date().toLocaleString('en-ZA', { timeZone: _tz() }) + ' · WorkClock-In by Reatlegile Solutions</div>';
  window.print();
}
function payrollCSV() {
...
// ── Bootstrap ─────────────────────────────────────────────
var APP_VERSION = 'v33';   // bump alongside the sw.js CACHE version on each deploy

-- ============================================================
--  WorkClock — record sign-in device on each clock-in
--  Incremental migration: run AFTER phase2b_worker_clock_rpc.sql.
--
--  Adds attendance.device_label and a 7th parameter (p_device_label)
--  to worker_clock_action so the browser's device label (OS · browser)
--  is stored on every clock-in. Safe / additive. The old 6-arg RPC is
--  dropped so only one signature exists (PostgREST dislikes overloads);
--  the new param defaults to NULL, so the currently-live frontend keeps
--  working until the updated frontend is deployed.
-- ============================================================
